...
public class WinGame_Event : Event
{
    ...
    [Tooltip("The win panel menu object")]
    [SerializeField] GameObject pnl_WinGame;
    ...
    [Tooltip("It will perform when the player wins. It should have infinite range and set to unscaled time. If you don't need it leave it empty")]
    ...
    private void Start()
    {
        //if the win panel was activated it hides it
        if (pnl_WinGame.activeSelf)
        {
            pnl_WinGame.SetActive(false);
        }
        players = GameObject.FindGameObjectsWithTag("Player");
    }
    public override void DoEvent()
    {
        //Show the win panel menu and make the player inmortal
        if (!pnl_WinGame.activeSelf)
        {
            pnl_WinGame.SetActive(true);
        }
        if(sound != null)
        {
